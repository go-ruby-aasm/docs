...
`go-ruby-aasm/aasm` reimplements Ruby's `aasm` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-aasm/aasm` directly, with no Ruby runtime;
...
Because `aasm` is pure Go with cgo disabled, it also compiles to **WebAssembly**
